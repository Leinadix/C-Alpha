...
Symbol* SymbolTable::findSymbol(const std::string& name) {
    ...
    std::cout << "Symbol '" << name << "' not found in any scope." << std::endl;
...
void SemanticAnalyzer::printSymbolTable() const {
    ...
    std::cout << "Current scope: " << symbolTable.getCurrentScopeName() << std::endl;
    ...
    for (const auto* scope : allScopes) {
        ...
        if (scope->symbols.empty()) {
            ...
            std::cout << "\nScope: " << scope->scopeName << std::endl;
            for (const auto& symbolPair : scope->symbols) {
                ...
                if (symbol->symbolKind == SymbolKind::VARIABLE) {
                    std::cout << " [" << (symbol->isInitialized ? "initialized" : "uninitialized") << "]";
                }
                std::cout << std::endl;
            }
        }
...
std::unique_ptr<SemanticType> SemanticAnalyzer::convertType(const Type* astType) {
    if (!astType) {
    ...
    switch (astType->nodeType) {
        case NodeType::BASIC_TYPE: {
            const BasicType* basicType = static_cast<const BasicType*>(astType);
        ...
        case NodeType::POINTER_TYPE: {
            const PointerType* ptrType = static_cast<const PointerType*>(astType);
        ...
        case NodeType::LAYOUT_TYPE: {
            const LayoutType* layoutType = static_cast<const LayoutType*>(astType);
            std::string layoutName = layoutType->layoutName;
            
            // Check if this is a namespace-qualified layout type (e.g., heap.ArrayHeader)
            ...
            if (dotPos != std::string::npos) {
                ...
                if (!layoutSymbol || layoutSymbol->symbolKind != SymbolKind::LAYOUT) {
                    addError("Undefined layout type '" + typeName + "' in namespace '" + namespaceName + "'",
...
void SemanticAnalyzer::visitStatement(const Statement* stmt) {
    ...
    switch (stmt->nodeType) {
        case NodeType::NAMESPACE_DECLARATION:
            visitNamespaceDeclaration(static_cast<const NamespaceDeclaration*>(stmt));
            break;
        ...
        case NodeType::LAYOUT_DECLARATION:
            visitLayoutDeclaration(static_cast<const LayoutDeclaration*>(stmt));
            break;
        case NodeType::ASSIGNMENT:
            visitAssignment(static_cast<const Assignment*>(stmt));
            break;
...
void SemanticAnalyzer::visitNamespaceDeclaration(const NamespaceDeclaration* namespaceDecl) {
...
void SemanticAnalyzer::visitVariableDeclaration(const VariableDeclaration* varDecl) {
    if (symbolTable.hasSymbolInCurrentScope(varDecl->name)) {
        addError("Variable '" + varDecl->name + "' already declared in current scope", 
    ...
    if (varDecl->initializer) {
        ...
        if (varDecl->initializer->nodeType == NodeType::STRING_LITERAL && 
            ...
            const PointerSemanticType* ptrType = static_cast<const PointerSemanticType*>(semanticType.get());
        ...
        // Check for invalid string literal assignment to single char
            ...
            addError("Cannot assign string literal to single char variable. Use single quotes for character literals (e.g., '\\n') or declare as char pointer (e.g., ->char)", 
                    varDecl->line, varDecl->column);
        }
        else if (initType && !semanticType->isCompatibleWith(initType.get())) {
            addError("Type mismatch in variable initialization for '" + varDecl->name + 
                    "'. Expected " + semanticType->toString() + ", got " + initType->toString(),
                    varDecl->line, varDecl->column);
        } else {
            isInitialized = true;
        }
    } else if (semanticType->isLayout()) {
        // Layout variables are considered initialized even without explicit initializer
        // This allows access to their members
        isInitialized = true;
    }
    
    auto symbol = std::make_unique<Symbol>(varDecl->name, SymbolKind::VARIABLE,
                                         std::move(semanticType), varDecl->line, varDecl->column,
                                         isInitialized);
    symbolTable.addSymbol(std::move(symbol));
}

void SemanticAnalyzer::visitFunctionDeclaration(const FunctionDeclaration* funcDecl) {
    if (symbolTable.hasSymbolInCurrentScope(funcDecl->name)) {
        addError("Function '" + funcDecl->name + "' already declared in current scope",
    ...
    auto funcType = std::make_unique<FunctionSemanticType>(returnType->clone(), std::move(paramTypes));
    
    auto symbol = std::make_unique<Symbol>(funcDecl->name, SymbolKind::FUNCTION,
                                         std::move(funcType), funcDecl->line, funcDecl->column, true);
    symbolTable.addSymbol(std::move(symbol));
    ...
    for (const auto& param : funcDecl->parameters) {
        ...
        auto paramSymbol = std::make_unique<Symbol>(param->name, SymbolKind::PARAMETER,
                                                  std::move(paramType), param->line, param->column, true);
...
void SemanticAnalyzer::visitLayoutDeclaration(const LayoutDeclaration* layoutDecl) {
    if (symbolTable.hasSymbolInCurrentScope(layoutDecl->name)) {
        addError("Layout '" + layoutDecl->name + "' already declared in current scope",
                layoutDecl->line, layoutDecl->column);
        return;
    }

    std::cout << "\n> Processing layout declaration: " << layoutDecl->name << " at line " << layoutDecl->line << ", column " << layoutDecl->column << std::endl;
    
    // Create a temporary symbol to get the FQDN that the symbol table will generate.
    // This is the most reliable way to get the canonical name.
    auto tempSymbolForFqdn = std::make_unique<Symbol>(layoutDecl->name, SymbolKind::LAYOUT, nullptr, layoutDecl->line, layoutDecl->column, true);
    ...
    // Always create a forward declaration first, then replace with complete layout
    // This ensures the layout type is available for processing members and other statements
    auto forwardLayoutType = std::make_unique<LayoutSemanticType>(fqdn, std::vector<std::unique_ptr<LayoutSemanticType::Member>>());
    auto forwardSymbol = std::make_unique<Symbol>(layoutDecl->name, SymbolKind::LAYOUT,
                                                 std::move(forwardLayoutType), layoutDecl->line, layoutDecl->column, true);
    symbolTable.addSymbol(std::move(forwardSymbol));



    std::cout << "\t> Forward layout declaration: " << fqdn << std::endl;
    fqdn = symbolTable.getScopes().back()->symbols[layoutDecl->name]->fqdn;
    std::cout << "\t> Finalized FQDN for layout: " << fqdn << std::endl;
    ...
    for (const auto& member : layoutDecl->members) {
        ...
        //std::cout << "\t[] Layout member: " << member->name << " of type " << members.back()->type->toString() << std::endl;
    }
    
    // Create the complete layout type
    auto layoutType = std::make_unique<LayoutSemanticType>(fqdn, std::move(members));

    // std::cout << "\t> Complete layout type: " << fqdn << std::endl;
    
    auto symbol = std::make_unique<Symbol>(layoutDecl->name, SymbolKind::LAYOUT,
                                         std::move(layoutType), layoutDecl->line, layoutDecl->column, true);
...
void SemanticAnalyzer::visitAssignment(const Assignment* assignment) {
    ...
    if (targetType && valueType) {
        ...
        if (assignment->value->nodeType == NodeType::STRING_LITERAL && 
            ...
            const PointerSemanticType* ptrType = static_cast<const PointerSemanticType*>(targetType.get());
            if (ptrType->pointsTo->kind == SemanticTypeKind::CHAR) {
        ...
        if (!targetType->isCompatibleWith(valueType.get())) {
            addError("Type mismatch in assignment. Expected " + targetType->toString() + 
...
void SemanticAnalyzer::visitIfStatement(const IfStatement* ifStmt) {
    ...
    if (condType && !condType->isNumeric()) {
        addError("If condition must be a numeric type", ifStmt->line, ifStmt->column);
    }
...
void SemanticAnalyzer::visitWhileStatement(const WhileStatement* whileStmt) {
    ...
    if (condType && !condType->isNumeric()) {
        addError("While condition must be a numeric type", whileStmt->line, whileStmt->column);
    }
...
void SemanticAnalyzer::visitReturnStatement(const ReturnStatement* retStmt) {
    if (!currentFunctionReturnType) {
        addError("Return statement outside of function", retStmt->line, retStmt->column);
        return;
    }
    
    if (retStmt->value) {
        auto valueType = visitExpression(retStmt->value.get());
        if (valueType && !currentFunctionReturnType->isCompatibleWith(valueType.get())) {
            addError("Return type mismatch. Expected " + currentFunctionReturnType->toString() + 
                    ", got " + valueType->toString(),
                    retStmt->line, retStmt->column);
        }
    } else {
        if (currentFunctionReturnType->kind != SemanticTypeKind::VOID) {
            addError("Missing return value. Expected " + currentFunctionReturnType->toString(),
                    retStmt->line, retStmt->column);
        }
    }
}
...
void SemanticAnalyzer::visitExpressionStatement(const ExpressionStatement* exprStmt) {
    visitExpression(exprStmt->expression.get());
}

std::unique_ptr<SemanticType> SemanticAnalyzer::visitExpression(const Expression* expr) {
    if (!expr) {
        return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
    }
    ...
    switch (expr->nodeType) {
        case NodeType::NAMESPACE_ACCESS:
            return visitNamespaceAccess(static_cast<const NamespaceAccess*>(expr));
            break;
        case NodeType::LITERAL:
            return visitLiteral(static_cast<const Literal*>(expr));
        case NodeType::STRING_LITERAL:
            return visitStringLiteral(static_cast<const StringLiteral*>(expr));
        case NodeType::IDENTIFIER:
            return visitIdentifier(static_cast<const Identifier*>(expr));
        case NodeType::BINARY_EXPRESSION:
            return visitBinaryExpression(static_cast<const BinaryExpression*>(expr));
        case NodeType::UNARY_EXPRESSION:
            return visitUnaryExpression(static_cast<const UnaryExpression*>(expr));
        case NodeType::FUNCTION_CALL:
            return visitFunctionCall(static_cast<const FunctionCall*>(expr));
        case NodeType::ARRAY_ALLOCATION:
            return visitArrayAllocation(static_cast<const ArrayAllocation*>(expr));
        case NodeType::ARRAY_ACCESS:
            return visitArrayAccess(static_cast<const ArrayAccess*>(expr));
        case NodeType::MEMBER_ACCESS:
            return visitMemberAccess(static_cast<const MemberAccess*>(expr));
        case NodeType::SYSCALL_EXPRESSION:
            return visitSyscallExpression(static_cast<const SyscallExpression*>(expr));
        case NodeType::TYPE_CAST:
            return visitTypeCast(static_cast<const TypeCast*>(expr));
        default:
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitNamespaceAccess(const NamespaceAccess* namespaceAccess) {
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitTypeCast(const TypeCast* typeCast) {
    ...
    if (!((targetType->isNumeric() && exprType->isNumeric())
        ...
        {
        addError("Type cast only supported between numeric types (int and char) or pointer types", 
                typeCast->line, typeCast->column);
        return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
    }
    
    // Warn about potential data loss when casting from int to char
    if (targetType->kind == SemanticTypeKind::CHAR && exprType->kind == SemanticTypeKind::INT) {
        std::cerr << "Warning: Possible data loss when casting from int to char at line " 
                  << typeCast->line << ", column " << typeCast->column << std::endl;
    }
    
    return targetType->clone();
}

std::unique_ptr<SemanticType> SemanticAnalyzer::visitLiteral(const Literal* literal) {
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitStringLiteral(const StringLiteral* stringLiteral) {
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitIdentifier(const Identifier* id) {
    Symbol* symbol = symbolTable.findSymbol(id->name);
    if (!symbol) {
    ...
    if (!symbol->isInitialized && symbol->symbolKind == SymbolKind::VARIABLE) {
        addError("Use of uninitialized variable '" + id->name + "'", id->line, id->column);
    }
    
    return symbol->type->clone();
}

std::unique_ptr<SemanticType> SemanticAnalyzer::visitBinaryExpression(const BinaryExpression* binExpr) {
    ...
    bool leftIsStringLiteral = (binExpr->left->nodeType == NodeType::STRING_LITERAL);
    bool rightIsStringLiteral = (binExpr->right->nodeType == NodeType::STRING_LITERAL);
    
    if ((leftIsChar && rightIsStringLiteral) || (rightIsChar && leftIsStringLiteral)) {
        if (binExpr->operator_ == TokenType::EQUAL || binExpr->operator_ == TokenType::NOT_EQUAL) {
            addError("Cannot compare char with string literal. Use single quotes for character comparison (e.g., '\\0' instead of \"\\0\")", 
    ...
    switch (binExpr->operator_) {
        ...
        case TokenType::MODULO:
            if (!leftType->isNumeric() || !rightType->isNumeric()) {
                addError("Arithmetic operators require numeric types", binExpr->line, binExpr->column);
                return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
            }
            // Result type is the "larger" of the two types (int > char)
            if (leftType->kind == SemanticTypeKind::INT || rightType->kind == SemanticTypeKind::INT) {
                return std::make_unique<BasicSemanticType>(SemanticTypeKind::INT);
            }
            return std::make_unique<BasicSemanticType>(SemanticTypeKind::CHAR);
        ...
        case TokenType::BITWISE_XOR:
            if (!leftType->isNumeric() || !rightType->isNumeric()) {
                addError("Bitwise operators require numeric types", binExpr->line, binExpr->column);
                return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
            }
        ...
        case TokenType::GREATER_EQUAL:
            ...
            if (!leftType->isCompatibleWith(rightType.get()) && !rightType->isCompatibleWith(leftType.get())) {
                addError("Cannot compare incompatible types: " + leftType->toString() + " and " + rightType->toString(), 
                        binExpr->line, binExpr->column);
                return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
            }
            return std::make_unique<BasicSemanticType>(SemanticTypeKind::INT);
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitUnaryExpression(const UnaryExpression* unExpr) {
    ...
    switch (unExpr->operator_) {
        ...
        case TokenType::BITWISE_NOT:
            if (!operandType->isNumeric()) {
                addError("Unary arithmetic operators require numeric types", unExpr->line, unExpr->column);
                return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
            }
        ...
        case TokenType::DEREFERENCE: {
            if (!operandType->isPointer()) {
                addError("Dereference operator requires pointer type", unExpr->line, unExpr->column);
                return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
            }
            const PointerSemanticType* ptrType = static_cast<const PointerSemanticType*>(operandType.get());
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitFunctionCall(const FunctionCall* funcCall) {
    ...
    if (dotPos != std::string::npos) {
        ...
        if (!symbol) {
            addError("Undefined function '" + functionName + "' in namespace '" + namespaceName + "'",
        ...
        if (!symbol->type->isFunction()) {
            addError("'" + functionName + "' in namespace '" + namespaceName + "' is not a function",
                    funcCall->line, funcCall->column);
            symbolTable.popScope();
            return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
        }
        
        const FunctionSemanticType* funcType = static_cast<const FunctionSemanticType*>(symbol->type.get());
        
        // Check arguments
        if (funcCall->arguments.size() != funcType->parameterTypes.size()) {
            addError("Function '" + namespaceName + "." + functionName + "' expects " +
                    std::to_string(funcType->parameterTypes.size()) + " arguments, got " +
        ...
        for (size_t i = 0; i < funcCall->arguments.size(); ++i) {
            ...
            if (argType && funcType->parameterTypes[i]) {
                if (!funcType->parameterTypes[i]->isCompatibleWith(argType.get())) {
                    addError("Argument " + std::to_string(i + 1) + " type mismatch in function '" +
                            namespaceName + "." + functionName + "'. Expected " +
                            funcType->parameterTypes[i]->toString() + ", got " + argType->toString(),
                            funcCall->line, funcCall->column);
                }
            }
        }
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitArrayAllocation(const ArrayAllocation* arrayAlloc) {
    auto sizeType = visitExpression(arrayAlloc->size.get());
    if (sizeType && !sizeType->isNumeric()) {
        addError("Array size must be numeric", arrayAlloc->line, arrayAlloc->column);
    }
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitArrayAccess(const ArrayAccess* arrayAccess) {
    ...
    if (indexType && !indexType->isNumeric()) {
        addError("Array index must be numeric", arrayAccess->line, arrayAccess->column);
    }
    ...
    if (!arrayType->isPointer() && !arrayType->isArray()) {
        addError("Array access requires pointer/array type", arrayAccess->line, arrayAccess->column);
        return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
    }
    
    if (arrayType->isPointer()) {
        const PointerSemanticType* ptrType = static_cast<const PointerSemanticType*>(arrayType.get());
        return ptrType->pointsTo->clone();
    } else if (arrayType->isArray()) {
        const ArraySemanticType* arrType = static_cast<const ArraySemanticType*>(arrayType.get());
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitMemberAccess(const MemberAccess* memberAccess) {
    ...
    if (objectType->isPointer()) {
        const PointerSemanticType* ptrType = static_cast<const PointerSemanticType*>(objectType.get());
        objectType = ptrType->pointsTo->clone();
    }
    
    if (!objectType->isLayout()) {
        addError("Member access requires layout type", memberAccess->line, memberAccess->column);
        return std::make_unique<BasicSemanticType>(SemanticTypeKind::ERROR);
    }
    
    const LayoutSemanticType* layoutType = static_cast<const LayoutSemanticType*>(objectType.get());
    const LayoutSemanticType::Member* member = layoutType->findMember(memberAccess->memberName);
    
    if (!member) {
        addError("Layout '" + layoutType->layoutName + "' has no member '" + memberAccess->memberName + "'",
...
std::unique_ptr<SemanticType> SemanticAnalyzer::visitSyscallExpression(const SyscallExpression* syscallExpr) {
    // syscall expects exactly 7 arguments (based on user example: syscall(0, 1, 2, 3, 4, 5, 8))
