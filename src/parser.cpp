...
std::unique_ptr<Type> Parser::parseType() {
    ...
    if (match(TokenType::REFERENCE)) {
        try {
            ...
            if (!pointsTo) {
                throw ParseError("Invalid type after '->' in pointer type", currentToken().line, currentToken().column);
            }
            return std::make_unique<PointerType>(std::move(pointsTo), startLine, startColumn);
        } catch (const ParseError& e) {
            // Add more context to the error
            throw ParseError("Invalid pointer type: " + std::string(e.what()) + 
                           "\nExpected a valid type after '->'", startLine, startColumn);
        }
    }
    
    // Handle basic types
    if (match(TokenType::INT)) {
        return std::make_unique<BasicType>(TokenType::INT, startLine, startColumn);
    }
    
    if (match(TokenType::CHAR)) {
        return std::make_unique<BasicType>(TokenType::CHAR, startLine, startColumn);
    }
    ...
    if (check(TokenType::IDENTIFIER)) {
        ...
        if (check(TokenType::DOT)) {
            ...
            if (!check(TokenType::IDENTIFIER)) {
                throw ParseError("Expected type name after namespace in '" + typeName + ".'",
    ...
    if (check(TokenType::STRING_LITERAL)) {
        throw ParseError("String literals cannot be used as types. Use 'char' for character type or '->char' for string pointer", 
                        startLine, startColumn);
    }
    
    if (check(TokenType::INTEGER)) {
        throw ParseError("Integer literals cannot be used as types. Use 'int' for integer type", 
                        startLine, startColumn);
    }
    ...
    if (inTypeCast) {
        throw ParseError("Expected type after '<' in type cast. Valid types are: int, char, layout name, or pointer type (->)", 
                        startLine, startColumn);
    } else {
        throw ParseError("Expected type (int, char, layout name, or pointer type starting with '->')", 
                        startLine, startColumn);
    }
}
...
std::unique_ptr<Expression> Parser::parseLogicalOr() {
    ...
    while (match(TokenType::BITWISE_OR)) {
        ...
        expr = std::make_unique<BinaryExpression>(std::move(expr), std::move(right), op, line, column);
    }
...
std::unique_ptr<Expression> Parser::parseUnary() {
    ...
    if (check(TokenType::BITWISE_NOT)) {
        // Look ahead to determine if this is array allocation (~Type[size]) or bitwise NOT
        if (position + 1 < tokens.size()) {
            TokenType nextToken = tokens[position + 1].type;
            // If ~ is followed by a type token, it's array allocation
            // This includes basic types (int, char), identifiers (layout names), and pointer types (->)
            if (nextToken == TokenType::INT || nextToken == TokenType::CHAR || 
                nextToken == TokenType::IDENTIFIER || nextToken == TokenType::REFERENCE) {
        ...
        advance(); // consume the ~
        auto expr = parseUnary();
        return std::make_unique<UnaryExpression>(std::move(expr), op, line, column);
    }
    
    // Handle special case for -> (could be reference operator or start of pointer type)
    if (check(TokenType::REFERENCE)) {
        // Look ahead to see if this is followed by a type token
        if (position + 1 < tokens.size()) {
            TokenType nextToken = tokens[position + 1].type;
            // If -> is followed by a type token, this might be a type, not a unary operator
            if (nextToken == TokenType::INT || nextToken == TokenType::CHAR || 
                (nextToken == TokenType::IDENTIFIER && position + 2 < tokens.size() && 
                 tokens[position + 2].type != TokenType::LEFT_PAREN)) {
                // This looks like a type (->int, ->char, ->LayoutName but not ->functionName())
                // Check if we're in a type cast context
                bool inTypeCast = false;
                if (position >= 2) {
                    TokenType prevToken = tokens[position - 1].type;
                ...
                throw ParseError("Unexpected pointer type in expression context. Did you mean to use this in a variable declaration?", 
                               currentToken().line, currentToken().column);
            }
        }
...
std::unique_ptr<Expression> Parser::parsePrimary() {
    ...
    if (match(TokenType::INTEGER)) {
        ...
        expr = std::make_unique<Literal>(token.value, TokenType::INTEGER, token.line, token.column);
    } else if (match(TokenType::CHARACTER)) {
        Token& token = tokens[position - 1];
        expr = std::make_unique<Literal>(token.value, TokenType::CHARACTER, token.line, token.column);
    } else if (match(TokenType::STRING_LITERAL)) {
        Token& token = tokens[position - 1];
        expr = std::make_unique<StringLiteral>(token.value, token.line, token.column);
        ...
        try {
            ...
            consume(TokenType::GREATER_THAN, "Expected '>' after type in type cast");
            consume(TokenType::LEFT_PAREN, "Expected '(' after type cast");
            auto expression = parseExpression();
            consume(TokenType::RIGHT_PAREN, "Expected ')' after cast expression");
            expr = std::make_unique<TypeCast>(std::move(targetType), std::move(expression), startLine, startColumn);
        } catch (const ParseError& e) {
            throw ParseError("Type cast error: " + std::string(e.what()), startLine, startColumn);
        }
        ...
        expr = std::make_unique<Identifier>(token.value, token.line, token.column);
        advance();
    } else if (match(TokenType::LEFT_PAREN)) {
        expr = parseExpression();
        consume(TokenType::RIGHT_PAREN, "Expected ')' after expression");
    } else {
        throw ParseError("Expected expression", currentToken().line, currentToken().column);
    }
    ...
    while (true) {
        if (check(TokenType::LEFT_PAREN)) {
            ...
            expr = std::make_unique<FunctionCall>(functionName, std::move(arguments), line, column);
        } else if (check(TokenType::LEFT_BRACKET)) {
            expr = parseArrayAccess(std::move(expr));
        } else if (check(TokenType::DOT)) {
            advance(); // consume '.'
            if (!check(TokenType::IDENTIFIER)) {
                throw ParseError("Expected member name after '.'", currentToken().line, currentToken().column);
            }
            Token& token = tokens[position];
            expr = std::make_unique<MemberAccess>(std::move(expr), token.value, token.line, token.column);
            advance();
        } else {
            break;
        }
...
std::unique_ptr<Statement> Parser::parseStatement() {
    ...
    try {
        ...
        if (check(TokenType::INT) || check(TokenType::CHAR) || check(TokenType::REFERENCE)) {
            return parseVariableDeclaration();
        }
        
        // Check for layout type variable declarations (identifier followed by identifier)
        if (check(TokenType::IDENTIFIER)) {
            // Look ahead to see if this is a variable declaration (identifier followed by identifier)
            if (position + 1 < tokens.size() && tokens[position + 1].type == TokenType::IDENTIFIER) {
                return parseVariableDeclaration();
            }
        }
        ...
        try {
            ...
            if (!expr) {
                ...
                throw ParseError("Failed to parse expression", currentToken().line, currentToken().column);
            }
            ...
            if (match(TokenType::ASSIGN)) {
                ...
                if (!value) {
                    position = savedPosition;
                    throw ParseError("Failed to parse assignment value", currentToken().line, currentToken().column);
                }
            ...
            // Special case: if we see a pointer type pattern in expression context, 
            // suggest it might be a missing variable declaration
            if (check(TokenType::REFERENCE) && position + 1 < tokens.size()) {
                TokenType nextToken = tokens[position + 1].type;
                if (nextToken == TokenType::INT || nextToken == TokenType::CHAR || nextToken == TokenType::IDENTIFIER) {
                    throw ParseError("Unexpected pointer type. Did you forget to declare this as a variable? (e.g., missing type before identifier)", 
                                   currentToken().line, currentToken().column);
                }
            }
            
            throw ParseError("Expected statement", currentToken().line, currentToken().column);
        }
    } catch (const ParseError& e) {
        // Re-throw with additional context
        throw ParseError("Statement parsing failed: " + std::string(e.what()), e.getLine(), e.getColumn());
    }
}
...
std::unique_ptr<Statement> Parser::parseVariableDeclaration() {
    ...
    if (match(TokenType::ASSIGN)) {
        try {
            ...
            if (!initializer) {
                throw ParseError("Invalid initializer expression", currentToken().line, currentToken().column);
            }
        } catch (const ParseError& e) {
            // Add more context to initialization errors
            throw ParseError("Error in variable initialization: " + std::string(e.what()), 
                           currentToken().line, currentToken().column);
        }
    }
    ...
    return std::make_unique<VariableDeclaration>(std::move(type), name, 
                                                std::move(initializer), startLine, startColumn);
}
...
std::unique_ptr<Statement> Parser::parseIfStatement() {
    ...
    return std::make_unique<IfStatement>(std::move(condition), std::move(thenStatement), 
...
std::unique_ptr<Statement> Parser::parseReturnStatement() {
    ...
    return std::make_unique<ReturnStatement>(std::move(value), startLine, startColumn);
}
...
std::unique_ptr<Statement> Parser::parseBlockStatement() {
    ...
    while (!check(TokenType::RIGHT_BRACE) && !isAtEnd()) {
        try {
            ...
            std::cerr << formatErrorMessage(e, "Parse Error in block: " + std::string(e.what())) << std::endl;
            ...
            // Ensure we've made progress - if not, force advance to avoid infinite loop
            if (position == errorPosition && !isAtEnd()) {
                std::cerr << "Warning: Error recovery stuck in block, forcing advance past token: " 
...
std::unique_ptr<Program> Parser::parseProgram() {
    ...
    while (!isAtEnd()) {
        try {
            ...
            // Print error for debugging with line content and highlighting
            std::cerr << formatErrorMessage(e, "Parse Error: " + std::string(e.what())) << std::endl;
            ...
            // Ensure we've made progress - if not, force advance to avoid infinite loop
            if (position == errorPosition && !isAtEnd()) {
                std::cerr << "Warning: Error recovery stuck, forcing advance past token: " 
                          << currentToken().value << std::endl;
                advance();
            }
...
std::unique_ptr<Statement> Parser::parseFunctionDeclaration() {
    ...
    // Parse function name
    if (!check(TokenType::IDENTIFIER)) {
        throw ParseError("Expected function name", currentToken().line, currentToken().column);
    }
    ...
    return std::make_unique<FunctionDeclaration>(std::move(returnType), name, 
...
std::unique_ptr<Statement> Parser::parseLayoutDeclaration() {
    ...
    // Parse layout name
    if (!check(TokenType::IDENTIFIER)) {
        throw ParseError("Expected layout name", currentToken().line, currentToken().column);
    }
    ...
    return std::make_unique<LayoutDeclaration>(name, std::move(members), startLine, startColumn);
}
...
std::unique_ptr<Parameter> Parser::parseParameter() {
    ...
    if (!check(TokenType::IDENTIFIER)) {
        throw ParseError("Expected parameter name", currentToken().line, currentToken().column);
    }
    
    std::string name = currentToken().value;
    advance();
    
    return std::make_unique<Parameter>(std::move(type), name, startLine, startColumn);
}
...
std::unique_ptr<LayoutMember> Parser::parseLayoutMember() {
    ...
    if (!check(TokenType::IDENTIFIER)) {
        throw ParseError("Expected member name", currentToken().line, currentToken().column);
    }
    ...
    return std::make_unique<LayoutMember>(std::move(type), name, startLine, startColumn);
}
...
std::unique_ptr<Expression> Parser::parseArrayAllocation() {
    ...
    // The ~ has already been consumed, now parse the element type (int, char, or layout type)
    auto elementType = parseType();
    
    consume(TokenType::LEFT_BRACKET, "Expected '[' after array type in ~Type[size] allocation");
    auto size = parseExpression();
    consume(TokenType::RIGHT_BRACKET, "Expected ']' after array size in ~Type[size] allocation");
    
    return std::make_unique<ArrayAllocation>(std::move(elementType), std::move(size), line, column);
}
...
std::unique_ptr<Expression> Parser::parseArrayAccess(std::unique_ptr<Expression> array) {
    ...
    return std::make_unique<ArrayAccess>(std::move(array), std::move(index), line, column);
}

std::unique_ptr<Expression> Parser::parseSyscallExpression(int line, int column) {
    consume(TokenType::LEFT_PAREN, "Expected '(' after 'syscall'");
    auto arguments = parseArgumentList();
    consume(TokenType::RIGHT_PAREN, "Expected ')' after syscall arguments");
    
    return std::make_unique<SyscallExpression>(std::move(arguments), line, column);
}
...
std::unique_ptr<Statement> Parser::parseImportStatement() {
    ...
    if (!check(TokenType::STRING_LITERAL)) {
        throw ParseError("Expected string literal for import path", currentToken().line, currentToken().column);
    }
...
std::unique_ptr<Statement> Parser::parseNamespaceDeclaration() {
    ...
    if (!check(TokenType::IDENTIFIER)) {
        throw ParseError("Expected namespace name", currentToken().line, currentToken().column);
    }
    ...
    return std::make_unique<NamespaceDeclaration>(name, std::move(statements), startLine, startColumn);
}

// Helper function to get line content and format error message with ANSI colors
std::string Parser::formatErrorMessage(const ParseError& e, const std::string& message) {
    // Find the token at the error position
    Token* errorToken = nullptr;
    for (size_t i = 0; i < tokens.size(); i++) {
        if (tokens[i].line == e.getLine() && tokens[i].column == e.getColumn()) {
            errorToken = &tokens[i];
            break;
        }
    }
    ...
    if (errorToken && !errorToken->sourceFile.empty()) {
        error << message << " in file '" << errorToken->sourceFile << "' at line " << e.getLine() << ", column " << e.getColumn() << "\n";
    } else {
        error << message << " at line " << e.getLine() << ", column " << e.getColumn() << "\n";
    }
    ...
    error << "  " << std::string(e.getColumn() - 1, ' ') << "\033[1;31m^\033[0m";
